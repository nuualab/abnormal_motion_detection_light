    ...
    for key in data:
        ...
        for idx, frame_num in enumerate(frame_num_list):
            ...
            try:
                ...
                data_after[key][frame_num]["human_confidence"] = human_conf_list_per_frame
                data_after[key][frame_num]["falldown_confidence"] = falldown_conf_list_per_frame
            ...
            for idx in object_idx_list:
                ...
                if h_confs[idx] >= h_threshold and y_confs[idx] >= y_threshold:
                    if f_confs[idx] >= f_threshold:
                        ...
                        f_confs_calibrated = calibrate_confidence(f_confs[idx], bbox, activation = "sigmoid", l2 = 0.8)
                    ...
                    if h_confs[idx] >= h_threshold and f_confs[idx] >= f_threshold:
                        ...
                        # y_confs를 보정
                        if calibrate_confidence(y_confs[idx], bbox, "sigmoid", l1 = 1, l2 = 2):
                            object_idx_list_filtered.append(idx)
                        else:
                        ...
                        h_conf_cal = calibrate_confidence(h_confs[idx], bbox, "sigmoid", l1 = 1, l2 = 2)
                        y_conf_cal = calibrate_confidence(y_confs[idx], bbox, "sigmoid", l1 = 1, l2 = 2)
    ...
    for i in range(len(indexs)-1):
        ...
        if keep[i] == True:
            for j in range(i + 1, len(indexs)):
                if compute_iou2(prepare_list[indexs[i]], prepare_list[indexs[j]]) > iou_threshold:
                    keep[j] = False
                    keep_num += 1
...
def create_main_bbox_process(data, **kwargs):
    ...
    for key in data_after:
        ...
        main_bbox, main_bbox_idx, main_bbox_conf = create_main_bbox(data_after, 
                                                                    key, 
    ...
    for key in data.keys():
        ...
        if len(object_list_bboxs) == 0:
            ...
            for idx in object_idx_list:
                ...
                data[key]["object"][idx]["object_yolo_confidence"] = object_detected_yolo_list
                data[key]["object"][idx]["object_human_confidence"] = object_detected_human_list
                data[key]["object"][idx]["object_falldown_confidence"] = object_detected_falldown_list
...
def interpolate_object_tracking_revised(data, inp_boundary_iou_threshold=0.3, inp_boundary_intersection_threshold=0.5, inp_boundary_time_threshold=12):
    ...
    for key in data_object_tracking.keys():
        ...
        for obj in data_object_tracking[key]["object"].keys():
            ...
            for idx in range(0, len(frame_list)):
                ...
                obj_bbox_i = data_object_tracking[key]["object"][obj]["object_bbox"][idx]

                obj_y_conf_i = data_object_tracking[key]["object"][obj]["object_yolo_confidence"][idx]
                obj_h_conf_i = data_object_tracking[key]["object"][obj]["object_human_confidence"][idx]
                obj_f_conf_i = data_object_tracking[key]["object"][obj]["object_falldown_confidence"][idx]
            ...
            condition2 = frame_list[-1] - frame_list[0] >= inp_boundary_time_threshold * 15
            ...
            if condition1 and condition2:
                for frame in range(frame_list[-1], original_frame_list[-1] + fps, fps):
                    ...
                    if len(bboxs) ==0:
                        isSuccess = False
                        frame_max = min(original_frame_list[-1] + fps, frame_list[-1] + fps)
                
                        for frame_ in range(frame + fps, frame_max, fps):
                            bboxs_ = data_object_tracking[key][frame_]["bbox"]
                            y_confs_ = data_object_tracking[key][frame_]["yolo_confidence"]
                            h_confs_ = data_object_tracking[key][frame_]["human_confidence"]
                            f_confs_ = data_object_tracking[key][frame_]["falldown_confidence"]
                            ...
                            for idx_, bbox_ in enumerate(bboxs_):
                                ...
                                intersection_ = compute_intersection_ratio(obj_bbox_list_new[-1], bbox_)
                                
                                condition1 = iou_ >= inp_boundary_iou_threshold
                                condition2 = intersection_ >= inp_boundary_intersection_threshold
                        ...
                        for idx, bbox in enumerate(bboxs):
                            ...
                            intersection = compute_intersection_ratio(obj_bbox_list_new[-1], bbox)
                            
                            condition1 = iou >= inp_boundary_iou_threshold
                            condition2 = intersection >= inp_boundary_intersection_threshold
                            condition3 = f_confs[idx] >= 0.8
            ...
            data_object_tracking[key]["object"][obj]["object_frame_list"] = frame_list_complete
            data_object_tracking[key]["object"][obj]["object_bbox"] = obj_bbox_list_new
            data_object_tracking[key]["object"][obj]["object_yolo_confidence"] = obj_y_conf_list_new
            data_object_tracking[key]["object"][obj]["object_human_confidence"] = obj_h_conf_list_new
            data_object_tracking[key]["object"][obj]["object_falldown_confidence"] = obj_f_conf_list_new
            
    return data_object_tracking

def interpolate_object_tracking(data, inp_boundary_iou_thres = 0.5, inp_boundary_intersection_thres = 0.5, inp_boundary_time_thres = 12):
                    ...
                    if len(bboxs) ==0 :
                        ...
                        for frame_ in range(frame + fps, frame_max, fps):
                            ...
                            for idx_, bbox_ in enumerate(bboxs_):
                                ...
                                condition2 = intersection_ >= inp_boundary_intersection_thres
...
def handling_boundary_frame(data, **kwargs):
    ...
    for key in data_object_tracking.keys():
        ...
        for obj in data_object_tracking[key]["object"].keys():
            ...
            frame_list = list(reversed(data_object_tracking[key]["object"][obj]["object_frame_list"].copy()))
            ...
            bboxs_list = list(reversed(data_object_tracking[key]["object"][obj]["object_bbox"].copy()))
            y_confs_list = list(reversed(data_object_tracking[key]["object"][obj]["object_yolo_confidence"].copy()))
            h_confs_list = list(reversed(data_object_tracking[key]["object"][obj]["object_human_confidence"].copy()))
            f_confs_list = list(reversed(data_object_tracking[key]["object"][obj]["object_falldown_confidence"].copy()))

            remove_count = 0

            while True:

                if f_confs_list[-1] < falldown_threshold and remove_count < remove_len_threshold:
            ...
            data_object_tracking[key]["object"][obj]["object_frame_list"] = list(reversed(frame_list))
            data_object_tracking[key]["object"][obj]["object_bbox"] = list(reversed(bboxs_list))
            data_object_tracking[key]["object"][obj]["object_yolo_confidence"] = list(reversed(y_confs_list))
            data_object_tracking[key]["object"][obj]["object_human_confidence"] = list(reversed(h_confs_list))
            data_object_tracking[key]["object"][obj]["object_falldown_confidence"] = list(reversed(f_confs_list))
...
def fill_result_all_frames(data_after):
    ...
    for key in data_final.keys():
        ...
        for idx, fld in enumerate(flds):
            ...
            if (idx + 1) % fps == 0:
                data_final[key]["annotations"][file_name]["bbox"] = data_final[key][(idx + 1)]["bbox"]   
            else:
                if int((idx + 1) // fps + 1) * fps > len(flds):
                    data_final[key]["annotations"][file_name]["bbox"] = data_final[key][int((idx + 1) // fps) * fps ]["bbox"] 
                else:
                    if idx + 1 < fps:
                        if len(data_final[key][fps]["bbox"]) >= 1:
                            data_final[key]["annotations"][file_name]["bbox"] = data_final[key][fps]["bbox"]
                        else:
...
def object_concat_per_bboxs(**kwargs):
    ...
    condition_1_in_2 = (x1_2 <= x2_2) and (x1_1 >=x2_1) and (y1_2 >= y2_2) and (y1_1 <=y2_1)
    condition_2_in_1 = (x2_2 <= x1_2) and (x2_1 >=x1_1) and (y2_2 >= y1_2) and (y2_1 <=y1_1)
    
    condition_1_close_2 = (x1_m <= x2_2) and (x1_m >=x2_1) and (y1_m >= y2_2) and (y1_m <=y2_1)
    condition_2_close_1 = (x2_m <= x1_2) and (x2_m >=x1_1) and (y2_m >= y1_2) and (y2_m <=y1_1)
    ...
    for idx in range(0, len(bboxs)):
        ...
        if True:
            ...
            for idx_ in range(idx+1, len(bboxs)):
                ...
                if (condition_iou or condition_intersection) and max_iou < intersection and condition_h_conf and condition_f_conf and condition_area:
    ...
    for key in data.keys():
        ...
        for frame in frame_list:
            ...
            if len(bboxs) == 0:
                ...
                bboxs_new, y_confs_new, h_confs_new, f_confs_new = object_concat_per_frame(bboxs = bboxs, 
                ...
                if bboxs_remove_list[idx] == 0:
                    ...
                    for idx_ in range(idx + 1, len(bboxs)):
                        ...
                        if bboxs_remove_list[idx_] == 0:
                            ...
                            condition_distance = ((box1[0] <= box2[0] and (box1[2] > box2[0] or box2[0] - box1[2] < distance_threshold)) or (box2[0] <= box1[0] and (box2[2] > box1[0] or box1[0] - box2[2] < distance_threshold))) and ((box1[1] <= box2[1] and (box1[3] > box2[1] or box2[1] - box1[3] < distance_threshold)) or (box2[1] <= box1[1] and (box2[3] > box1[1] or box1[1] - box2[3] < distance_threshold)))
        ...
        for frame in frame_list:
            ...
            for idx in range(0, len(bboxs)):
                ...
                for idx_ in range(0, len(bboxs)):
                    ...
                    if (condition_iou or condition_intersection) and area + 0.0001 < area_:
                        isRemove = True
                        break
...
if __name__ == '__main__':
    ...
    # yolo process
    data = yolo_process(folder, model_y_new, device, fps = fps, img_row = img_row, conf_thres = conf_thres, max_det = max_det)
    
    # human_cls
    data = human_classification_process(data, classifier = None)

    # falldown_cls
    data = falldown_classification_process(data, classifier_list["falldown_classification"])

    #build new structure
    data_branch1 = build_own_structure(data)

    # data handling
    data_branch2 = data_handling(data_branch1, 
    ...
    # create main bbox
    data_branch2 = create_main_bbox_process(data_branch2, 
    ...
    # object tracking
    data_branch2 = object_tracking(data_branch2, 
    ...
    data_branch3 = interpolate_object_tracking_revised(data_branch2, 
    ...
    data_branch4 = handling_boundary_frame(data_branch3, 
    ...
    # object concat
    data_branch5 = object_concat(data_branch4, 
                                 concat_iou_threshold = concat_iou_threshold,
                                 concat_intersection_threshold = concat_intersection_threshold,
                                 concat_human_threshold = concat_human_threshold, 
                                 concat_falldown_threshold = concat_falldown_threshold)
    
    data_branch5_ = object_concat_next_step(data_branch5, iou_threshold = 0.001, distance_threshold = 10)
    
    data_branch6 = remove_conjugated_object(data_branch5_, iou_threshold = 0.1, intersection_threshold = 0.1)
